...
class CountryApp extends Component {
  ...
  handleVisit = countryId => {
    this.setState(prevState => {
      const updatedCountries = prevState.countries.map(country =>
        country.id === countryId ? {...country, isVisited: true} : country,
      )

      const countryToAdd = prevState.countries.find(
        country => country.id === countryId,
      )

      return {
        countries: updatedCountries,
        visitedCountries: [...prevState.visitedCountries, countryToAdd],
      }
    })
  }

  handleRemove = countryId => {
    this.setState(prevState => {
      const updatedCountries = prevState.countries.map(country =>
        country.id === countryId ? {...country, isVisited: false} : country,
      )
